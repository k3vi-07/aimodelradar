# 9 月免费大模型 API 梳理：27 个基础快照可测试，11 个需核实，10 个暂不推荐（新增：本月 AI 大事件月历）
...
| 魔搭"新计费机制、旧免费额度口径失效" | **官方规则确认：新机制即"魔粒体系"**（短期魔粒 24h、长期魔粒 90 天，社区行为获取，适用 API-Inference；官方规则页 9-21 读者转达）——"每日 2000 次"旧口径确认作废 | ✅ 谜底揭晓：魔粒制（详见第五章） |

> 🔧 **上期勘误**：① "腾讯 TokenHub"上期标注为待核实，本期已通过腾讯云官方文档确认真实存在且为混元开放平台的升级形态（见政策变化第 2 条）；② 上期称 Cloudflare "Kimi 全系付费"，经官方计价表复核，**kimi-k2.5 不在付费专属名单内，免费 Neurons 可跑**；③ 上期"魔搭新计费机制"**已确认为魔粒体系**（见第五章），"每日 2000 次"旧口径作废。
...
### 6. 红名单修正：AI21 复活，DeepSeek 赠额说法删除

- **AI21**（上期红名单"官方定价页未核实到免费"）：官方定价页 9-21 直读现挂 **"$10 credits for 7 days, No credit card needed"**——限时试用版免费渠道，本期从红名单除名，改列国外赠金组。
- **DeepSeek 官方**（上期红名单"无免费 API"）：9 月曾有两条社区信源称新用户有数百万元级 token 赠额，**官方 api-docs 定价页未列明、无法证实——本期按官方口径删除该说法，维持红名单**（纯按量付费）。
...
## 二、基础快照总览（2026-09-21）：✅ 27 个可测试 / ⚠️ 11 个需核实 / ❌ 10 个暂不推荐
...
| ✅ | **ModelScope 魔搭** | **魔粒体系**：社区行为获取算力额度 | 循环额度（魔粒） | 带闪电标识开源模型（Qwen/DeepSeek/GLM/Kimi 等） | 短期魔粒 24h / 长期魔粒 90 天；完善信息+邮箱验证 | 🔄 魔粒机制官方确认回归 |
...
| ⚠️ | **白山智算 / PPIO / Nebius / Anthropic / MiniMax / 阶跃 / 面壁 / 天工 / 零一 / Novita / Together** | 详见第七章存疑清单（共 11 个） | — | — | — | — |
...
- **涨价预警（9-21 代理直读定价页）**：付费档 $0.75/$3.75 的介绍价**只到 2026-12-31**，2027-01-01 起翻倍至 $1.50/$7.50——重度用户赶早。
...
- **信源说明**：官方文档对脚本、无头浏览器与读者浏览器均返回 403/Forbidden（9-21 双向复核确认，需登录态访问），以上限额为多源一致口径，以登录后的官方页为准。
...
| **百度千帆** | 每模型 100 万 token（✅官方帮助文档直读 9-21：ERNIE-4.5-Turbo 系、ERNIE-X1-Turbo、DeepSeek-R1/V3 系等各 100 万、3 个月）；**ERNIE-Speed-8K/128K 各 100 万 token、仅 1 个月有效**（官方客服澄清：非永久免费，到期按刊例价计费） | 多数 3 个月 · Speed 1 个月 | ERNIE-4.5-Turbo、DeepSeek-R1/V3.1、ERNIE-Speed | 百度智能云实名 | TokenPlan 转积分制 |
...
### 2. ModelScope 魔搭 —— 谜底揭晓：新计费即"魔粒体系"（官方规则确认）

- **机制**（官方规则页，9-21 读者转达）：魔搭通过**魔粒体系**提供免费算力额度——**短期魔粒生效 24 小时、长期魔粒生效 90 天**，通过社区行为（体验模型、创作、互动、贡献）获取，可用于 **AIGC 推理服务、训练服务、API-Inference** 等场景。
- **前置要求**：完善个人信息，包括验证通过的个人邮箱。
- **口径结论**：上期"新计费机制"即魔粒体系，本期确认；社区流传的"每日 2000 次免费"为旧机制口径，**已作废**。具体魔粒获取量与消耗折算以控制台实时显示为准；规则动态更新，最终解释权归平台。
- **官网来源**：魔搭魔粒规则页 ✅（读者转达）｜https://www.modelscope.cn/docs/model-service/API-Inference/limits
...
### GLM-5.3 系列：智谱官方 vs 聚合平台

| 渠道 | 计费 | 输入 | 输出 | 备注 |
|---|---|---|---|---|
| 智谱官方 `GLM-5.3` | 按量 | ¥8/百万 | ¥28/百万 | 缓存命中 ¥2；缓存存储限时免费 ✅定价页 |
| OpenRouter `z-ai/glm-5.3` | 按量 | $0.91（≈¥6.5） | $2.86（≈¥20） | **比官方便宜约 20–30%** ✅API |
| 智谱官方 `GLM-5.3-Flash` | 按量 | ¥0.8 | ¥2.8 | ✅定价页 |
| OpenRouter `glm-5.3-flash` | 按量 | $0.09（≈¥0.64） | $0.30（≈¥2.1） | 略低于官方 ✅API |
...
| 渠道 | 输入 | 输出 | 备注 |
|---|---|---|---|
| Kimi 官方 `kimi-k3` | ¥20/百万 | ¥100/百万 | 输入未命中缓存价 ✅定价页 |
| OpenRouter `kimi-k3` | $1.70（≈¥12） | $8.50（≈¥60） | **比官方便宜约 40%**；batch 档反而 $3/$15 ✅API |
...
| Cerebras | Qwen3.8-27B | $0.99（≈¥7） | $1.49（≈¥10.6） | ~1850 tokens/s——**为速度付溢价**，输入价各家最贵 ✅官方定价页 |

### 旗舰闭源三连（付费参照系）与开源免费赛点

| 模型 | 官方直连价 | OpenRouter | 备注 |
|---|---|---|---|
...
| MiniMax-M3 | 官方平台刊例待核（⚠️ 控制台） | $0.30/$1.20（≈¥2.1/¥8.5） | ✅API |
| 阶跃 Step-3.7-Flash | 官方平台刊例待核（⚠️） | $0.20/$1.15（≈¥1.4/¥8.2）；3.5-Flash $0.10/$0.30 | ✅API |
...
> **腾讯 TokenHub 待补**：托管模型刊例价（Hy4-Preview、第三方 GLM/DeepSeek 等）**需登录后在模型详情页逐个查看，无公开汇总表**（读者 9-21 登录核实）——登录后看到数字即可按下表格式补录。
...
| **百度千帆** | 每模型 100 万×N（多数 3 个月 · Speed 系 1 个月） | 多模型轮换 | 多模型对比 | ★★★☆☆ |
...
> 本月红名单变动：**AI21 除名**（官方 $10/7 天试用确认）；**DeepSeek 官方**的社区赠额说法未获官方证实，按官方定价页口径**回到红名单**。
...
**本月核查方法说明**：海外平台以官方页直读 + OpenRouter 官方 API 实时拉取为主；国内平台以官方文档/定价页直读为主，社区信源仅作交叉验证且均已标注。**魔搭魔粒机制确认（9-21，官方规则页经读者转达）**：新计费即魔粒体系——短期魔粒 24 小时、长期魔粒 90 天，社区行为获取，适用 API-Inference；"每日 2000 次"旧口径作废，魔搭回归 ✅。

**ERNIE-Speed 口径澄清（9-21，官方客服经读者转达）**：非永久免费——8K/128K 各 100 万 token、领取后 1 个月有效，到期或用尽按刊例价计费，"永久免费不限量 RPM=300"的社区口径不成立，已全文修正。

**9 月 21 日代理复核轮**：经本地代理访问 Mistral/Anthropic/Jina/Gemini 官方页——Mistral $10/月 API 信用、Jina 100 RPM/100K TPM、Anthropic 赠金存在均获官方原文确认，Gemini 免费限额表确认已转 AI Studio 动态显示。另：对 curl 只能取到 JS 空壳的页面（火山、讯飞、白山、LongCat、Groq、Gemini）改用无头浏览器渲染复核——火山福利页与讯飞活动额度由此获得官方口径；千帆、百川官方文档直读升级；白山确认活动已从官网撤下；LongCat 平台页为美团登录墙——**读者当日登录核实：无每日免费额度，仅新用户 1000 万 + 邀请双方各 1000 万，本刊已据此修正**；Groq 文档反爬（对无头浏览器同样 Forbidden）、Gemini 域名在本核查环境网络不可达，这两家维持分层标注。免费政策变化很快，收藏时请同时记下核查日期。
