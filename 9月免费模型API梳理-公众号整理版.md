# 9 月免费大模型 API 梳理：23 个基础快照可测试，16 个需核实，9 个暂不推荐（新增：本月 AI 大事件月历）
...
官方定价页（9-21 实测）现役清单：`kimi-k3`（1M 上下文，输入未命中 ¥20/输出 ¥100 每百万）、`kimi-k2.6`（¥6.5/¥27）、`kimi-k2.7-code`、`kimi-k2.7-code-highspeed`。上期的"新用户 15 元代金券"**官方首页与定价页均已不见横幅**，社区（9-11）称仍在发放——口径待核，注册后以控制台为准。文件内容抽取/存储接口限时免费。K3 权重 7-27 已在 HuggingFace 开放，可自部署。
...
## 二、基础快照总览（2026-09-21）：✅ 23 个可测试 / ⚠️ 16 个需核实 / ❌ 9 个暂不推荐
...
| ✅ | **火山引擎·豆包** | 每模型 50 万 token + 协作奖励 | 赠金+循环 | doubao-seed-evolving、seed-2-1 系列 12 款 | 协作奖励第二期**单模型最高 500 万/日**，需授权数据训练 | 🆕 循环额度加码 |
...
| ✅ | **百度千帆** | 每模型 100 万 token | 一次性赠金 | ERNIE-4.5-Turbo、DeepSeek-V4、Qwen3；**ERNIE-Speed 永久免费不限量 RPM=300** | 3 个月有效 | TokenPlan 个人版转积分制（9 月中） |
...
| ✅ | **讯飞星火** | Lite 永久免费 + 活动额度 | 永久免费+活动 | Spark Lite（免费）、Spark4.0 Ultra（活动额度待核） | 活动口径 9 月未获官方复核 | 维持 |
...
| ⚠️ | **Kimi 开放平台** | 15 元代金券（官方页已无横幅） | 一次性赠金 | kimi-k3、k2.6、k2.7-code | 代金券状态待核 | 降级待核 |
| ⚠️ | **百川智能 / 白山智算 / PPIO / Nebius / Mistral / Anthropic / ModelScope / MiniMax / DeepSeek 官方 / 阶跃 / 面壁 / 天工 / 零一 / Novita / Together** | 详见第七章存疑清单（共 16 个） | — | — | — | — |
| ❌ | **Cerebras / GitHub Models / 无问芯穹 / 国家超算互联网 / OpenAI / xAI / DeepInfra / Replicate / 302.AI 系** | 详见第八章红名单（共 9 条） | — | — | — | AI21 除名、DeepSeek 移存疑 |

> ⚠️ 存疑条目完整清单见第七章，共 **16 个**；❌ 暂不推荐条目见第八章，共 **9 条**。"暂不推荐"只表示截至核查日不适合作为免费 API 方案，不代表平台本身不可用。
> 与 8 月对比：✅ 29 → 23（Cerebras 死亡、Kimi/百川/白山/PPIO/Nebius 降级待核，AI21/LongCat/TokenHub 入列）；免费渠道的总供给量并未下降，主要变化是**确定性收敛**——能官方核实口径的变少了。
...
- **注意**：以上数字以控制台实时显示为准；每日刷新意味着适合挂持续小任务，不适合一次性超大任务。
...
| **火山引擎·豆包** | 每模型 50 万 token + 协作奖励第二期**单模型最高 500 万 token/日** | 长期+每日 | doubao-seed-evolving、seed-2-1 系列 12 款 | 注册+实名；协作奖励需授权数据 | 🆕 循环加码 |
| **阿里云百炼** | 每模型 100 万 token | 90 天；仅北京地域 | qwen-max、qwen3.8、qwen-vl-max-latest | 注册自动发放 | 无变化 |
| **百度千帆** | 每模型 100 万 token；**ERNIE-Speed 永久免费不限量** | 3 个月 | ERNIE-4.5-Turbo、DeepSeek-V4、Qwen3 | 百度智能云实名 | TokenPlan 转积分制 |
| **硅基流动** | 新用户 14 元 + L0 免费档 16 款 ¥0 模型 | — | Qwen3-8B、GLM-4-9B、Xing4.0-29B-A4B（新） | 注册（大陆手机号） | 🆕 新模型 |
| **百川智能** ⚠️ | 新用户 80 元 | 3 个月 | Baichuan-M3 系、M3-Plus（API 降价 70%） | 注册+实名 | 官方页未直读，降级待核 |
| **Kimi 开放平台** ⚠️ | 15 元代金券（官方页已无横幅） | — | kimi-k3、k2.6、k2.7-code | 实名 | 降级待核 |
| **讯飞星火** | Spark Lite 永久免费；活动额度口径待核 | — | Spark Lite、Spark4.0 Ultra | 注册+实名 | 活动口径未证实 |
...
- **火山协作奖励**的 500 万/日不是白给的：机制是授权推理数据用于模型优化、T+1 按采集量返还。**敏感数据勿走此通道**。
...
| **Kimi 开放平台** | 15 元代金券：CSDN 9-11 称仍在（"永久有效"）；官方首页/定价页已无横幅 | 代金券存续、K3 是否参与 | platform.kimi.com（9-21 直读） |
| **百川智能** | 80 元/3 个月（社区 8-25 详述）；M3-Plus 发布且 API 降价 70% | 官方定价页未能直读 | platform.baichuan-ai.com |
| **白山智算** | "+300 元"部分 2026-03-15 已过期；官网 2-27 有"福利政策调整"公告 | 150 元与永久免费模型现行状态 | ai.baishan.com |
...
**Q1：8 月说 29 个可测试，9 月变 23 个，免费渠道在萎缩吗？**
渠道总量没少多少，**少的是"确定性"**：Kimi、百川、白山、PPIO、Nebius 这几家从"官方可核实"退到"社区口径待核"，Cerebras 彻底转付费。同时 LongCat、TokenHub、AI21 是净新增。真正的趋势是：永久免费层向头部集中（Gemini/智谱/Groq/Cloudflare），其余玩家转向试用金和积分制。
...
**本月核查方法说明**：海外平台以官方页直读 + OpenRouter 官方 API 实时拉取为主；国内平台以官方文档/定价页直读为主，社区信源仅作交叉验证且均已标注。部分平台官方页因网络/登录墙不可达，相应条目已降级标注。免费政策变化很快，收藏时请同时记下核查日期。
...
**火山豆包**
https://www.volcengine.com/docs/87301/2106521（协作奖励第二期社区多源）

**腾讯 TokenHub**
https://cloud.tencent.com/document/product/1823/130053 ✅（9-04 官方文档）

**百度千帆**
https://cloud.baidu.com（TokenPlan 积分制为社区源，约 9-15）
