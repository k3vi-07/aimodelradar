# 8 月免费大模型 API 梳理：29 个基础快照可测试，13 个需核实，17 个暂不推荐
...
| NVIDIA NIM | 50+ Free Endpoint | 动态目录，数量每日可能变化 | 具体可用性以模型页和账号控制台为准 |
...
## 二、基础快照总览（2026-08-19）：✅ 29 个可测试 / ⚠️ 13 个需核实 / ❌ 17 个暂不推荐
...
| ✅ | **NVIDIA NIM** | 50+ Free Endpoint | 动态免费端点 | DeepSeek-V4-Flash、Nemotron、GLM-5.2、MiniMax-M3、Cosmos | 数量和限制以目录为准 |
...
### 5. NVIDIA NIM —— 50+ Free Endpoint

- **免费模型数量**：NVIDIA 模型目录长期维持 **50+ Free Endpoint** 的动态规模。目录中可见 DeepSeek-V4-Flash、Nemotron、GLM-5.2、MiniMax-M3、Cosmos、Gemma、Step、Inkling、Laguna 等模型和服务。
- **注意**：数量会随目录和活动变化，不等于所有端点都是文本对话模型；其中包含视觉、视频、翻译、Embedding、安全检测等端点。具体可用性、速率和是否需要申请，以[官方模型目录](https://build.nvidia.com/models)和账号控制台为准。
